...
export function BestsellersSection() {
  ...
  function getVisibleCardsCount(scroller: HTMLDivElement, cards: HTMLElement[]): number {
    if (!cards.length) {
      return 1;
    }
    ...
    const step = secondCard
      ...
      : cardWidth + gap;
    const visibleByViewport = Math.round((scroller.clientWidth + gap) / Math.max(step, 1));

    return Math.min(
      cards.length,
      Math.max(1, visibleByViewport)
    );
  }
